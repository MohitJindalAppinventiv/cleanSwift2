...
export const CustomersTable = ({
    ...
    return (
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-6">
        <div className="text-sm text-gray-600 px-4 py-2 rounded-md border bg-white shadow-sm">
          ...
          of <span className="font-semibold text-gray-800">{filteredCustomers.length}</span>{" "}
          results ({limit} per page)
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Show pagination only if more pages exist and results are >= limit */}
      {filteredCustomers.length > 0 && totalPages > 1 && filteredCustomers.length >= limit && renderPagination()}
