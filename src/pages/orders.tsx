...
import { Badge } from "@/components/ui/badge";
import { Card, CardContent } from "@/components/ui/card";
import { Search, Package, Clock, CheckCircle, XCircle, Users } from "lucide-react";
import { fetchOrders } from "@/store/slices/orderSlice";
import { CalendarDateRangePicker } from "@/components/date-range-picker";
import { format } from "date-fns";
...
const OrdersPage = () => {
  const dispatch = useAppDispatch();
  const { orders = [], pagination, analytics, isLoading, error } = useAppSelector(
    (state) => state.orders
  );
  ...
  const fetchParams = useMemo((): FetchOrdersParams => {
    ...
    return params;
  }, [currentPage, pageSize, statusFilter, paymentMethodFilter, searchQuery, dateRange]);
  ...
  const hasActiveFilters = statusFilter !== 'all' || paymentMethodFilter !== 'all' || dateRange.startDate || dateRange.endDate;
